...
比如当收到指责时的反应模式，当听到不同声音时的行为模式，当发现上级的决定有可能是错误的时的行为模式，当发现别人的开发进度很慢时的行为模式，它体现了团队的价值观。想一想，这些场景是无法被穷举的，虽然我们有各种标准工作流程，组织架构，但是依然无法也不可能全部定义清楚，因此它是我们管理的重要补充。当然和上面的#管理比起来，它有一个很重要的特点，就是没法立竿见影。模式的行程需要时间，团队达成统一认知需要时间。
